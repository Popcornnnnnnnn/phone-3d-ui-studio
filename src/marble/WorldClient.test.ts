...
function setup() {
  ...
  c.receive({ type: 'world-welcome', protocolVersion: 1, worldId: 'fixture-world', clientId: 'fixture-browser' })
  return { c, sent }
}
describe('shared world presentation', () => {
  it('matches the native projection fixtures including partial entry and the right edge', () => {
    for (const fixture of fixtures) {
      ...
      const p = projectMarble(s.ball!, s.phone!, s.geometry)
  ...
  it('interpolates phone and ball at the same 50 ms presentation time without extrapolation', () => {
    ...
    b.sequence = 2; b.serverTimeMs = 1100; b.ball!.position[0] = 0.02; b.phone!.position[0] = 0.01
    c.receive(a, 1000); c.receive(b, 1100)
    expect(c.render(1100)?.ball?.position[0]).toBeCloseTo(0.01, 8)
    expect(c.render(1100)?.phone?.position[0]).toBeCloseTo(0.005, 8)
    expect(c.render(1200)?.ball?.position[0]).toBeCloseTo(0.02, 8)
  })
  ...
  it('freezes a stale display and requires a new round instead of accepting a catch-up jump', () => {
    ...
    const b = initial(); b.sequence = 2; b.serverTimeMs = 1300; b.ball!.position[0] = 0.2
    c.receive(b, 1300)
    expect(c.needsRestart).toBe(true); expect(c.render(1300)?.ball?.position[0]).toBe(0)
