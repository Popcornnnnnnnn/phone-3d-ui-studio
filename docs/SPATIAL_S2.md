...
> 历史 S2 回退版本。当前主体验为 [S2.1 无线弹力托盘](ELASTIC_TRAY.md)，已移除 Return。以下记录保留原样。
...
画布右下角 **Full screen** 可展开 3D 工作区；底部保留 Start/Return/Reset/Pause 与接球计数。右上角 **Exit full screen** 或 Esc 退出。切换尺寸沿用同一个 Canvas 和观察相机，只有 Reset view 主动复位视角。Tracking 页面也提供相同入口，并保留 Set origin。

Shift + 左键拖动（或右键拖动）平移，滚轮缩放。网格覆盖随观察位置延展，移除原先 3 m 的拉远限制；近处网格仍为 10 cm，远处按十倍尺度简化。这里扩展的是观察画布，S2 弹珠的碰撞地面、接球区与越界重置规则保持原先范围。

优先使用浏览器原生全屏；嵌入环境禁用此能力时占满当前页面窗口。按用户要求，后续全屏、Esc 退出和全屏内控制体验由用户手动验证，不再在 Codex 中操作全屏。Web 改动无需重新安装 iPhone App。
