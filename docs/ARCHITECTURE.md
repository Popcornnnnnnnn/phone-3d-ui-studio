...
`MarbleService` lazily initializes pinned `@dimforge/rapier3d-compat@0.20.0`. `MarbleWorld` uses 120 Hz fixed steps, 1.5 m/s² gravity, 7.5 mm radius spheres and the existing shallow rounded tray with a 40 mm right exit. The tray receives one next-step kinematic transform per 120 Hz physics step from `TrayMotion`. It keeps the phone sampling timeline anchored at calibration, buffers 40 ms, and filters translation/rotation with a 25 ms exponential time constant. On input underrun the sampling cursor holds; delayed packets never accelerate playback. No future motion is predicted. Gaps over 250 ms or implausible jumps pause and require recalibration. The existing 50 ms shared rendering buffer remains in addition to this input delay. Restitution is 0.5 for the active ball and floor, 0.2 for rails, 0.1 for the ground, with Min combination. Linear/angular damping dissipates energy.

Each explicit addition creates a new UUID at the current calibrated tray center; only one ball is active. Actual front-floor contact after a clear departure (over 2 mm or beyond the screen edge) and at least 40 ms separation increments a sequenced impact event. Rapier lengthUnit is the 15 mm ball diameter; contact receipts require solver separation at most 0.3 mm. This avoids mistaking distant predictive contacts or tiny resting gaps for catches. Resting contact cannot repeatedly vibrate. First ground contact retires the ball and restricts collisions to ground only. After settling it becomes a saved visual pose with no rigid body. At five retained balls, addition removes the oldest ground record. Out-of-bounds balls are removed and require explicit replacement. There is no Return, attraction, automatic launch or automatic reset.
